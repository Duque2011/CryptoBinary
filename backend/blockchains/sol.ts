...
class SolanaService {
  ...
  private static readonly PROCESSING_EXPIRY_MS: number = 30 * 60 * 1000;

  private static cleanupProcessedTransactions() {
    const now = Date.now();
    for (const [sig, ts] of SolanaService.processedTransactions.entries()) {
      if (now - ts > SolanaService.PROCESSING_EXPIRY_MS) {
        SolanaService.processedTransactions.delete(sig);
      }
    }
  }
  ...
  public static async getInstance(): Promise<SolanaService> {
    if (!SolanaService.instance) {
      ...
      setInterval(
        () => SolanaService.cleanupProcessedTransactions(),
        60 * 1000
      );
    }
  ...
  async monitorSolanaDeposits(wallet: walletAttributes, address: string) {
    try {
      ...
      // Subscribe to account logs without removing the listener upon a single deposit.
      const logsSubscriptionId = await this.connection.onLogs(
        publicKey,
        async (logs, context) => {
          try {
            console.log(
            ...
            if (SolanaService.processedTransactions.has(transactionSignature)) {
              console.log(
                `[INFO] Transaction ${transactionSignature} already processed; ignoring duplicate event.`
              );
              return;
            }
            console.log(
            ...
            // Process and track the transaction
            ...
            console.error(
              `[ERROR] Error in log listener for ${address}: ${err.message}`
            );
          }
        },
        "confirmed"
  ...
  async trackTransactionSignature(
    ...
    try {
      ...
      if (SolanaService.processedTransactions.has(signature)) {
        console.log(
          `[INFO] Transaction ${signature} already processed; skipping.`
        );
        return;
      }
      const maxRetries = 30;
      ...
      while (retries < maxRetries) {
        try {
          ...
          if (transaction) {
            ...
            console.log(
              `[INFO] Transaction ${signature} not found. Retrying... (${retries + 1}/${maxRetries})`
            );
          }
        } catch (error) {
          console.error(
      ...
      const txDetails = {
        ...
        hash: signature,
        type: "DEPOSIT",
        from: "N/A", // Optionally extract sender if needed
        address: address,
      ...
      console.log(
        `[SUCCESS] Processed Solana transaction ${signature} for wallet ${wallet.id}`
      );
      // Mark transaction as processed with current timestamp
      SolanaService.processedTransactions.set(signature, Date.now());
    } catch (error) {
      console.error(
